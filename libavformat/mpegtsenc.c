...
#define MPEGTS_FLAG_DISCONT         0x10
    ...
    double scte35_period;
    int scte35_packet_count;
    int scte35_packet_period;
...
static int mpegts_init(AVFormatContext *s)
{
    ...
    if (ts->mux_rate > 1) {
        ...
        ts->scte35_packet_period = 100; // NETINT: add scte35 type to mpegts muxer as PSI
    ...
    // NETINT: add scte35 type to mpegts muxer as PSI
    if (ts->scte35_period < INT_MAX/2) {
        ts->scte35_packet_period = INT_MAX;
    }
    // output a PCR as soon as possible
    service->pcr_packet_count = service->pcr_packet_period;
    ts->pat_packet_count      = ts->pat_packet_period - 1;
    ts->sdt_packet_count      = ts->sdt_packet_period - 1;
    ts->scte35_packet_count   = ts->scte35_packet_period - 1; // NETINT: add scte35 type to mpegts muxer as PSI
