...
static int ni_logan_device_create(AVHWDeviceContext *ctx,
                            ...
                            int flags)
{
  AVNILOGANDeviceContext *ni_ctx;
  int ret = 0;

  av_log(ctx, AV_LOG_VERBOSE, "ni_logan_device_create %s\n", device);

  ni_ctx = (AVNILOGANDeviceContext *) ctx->hwctx;
...
static int ni_logan_get_buffer(AVHWFramesContext *ctx, AVFrame *frame)
{
  ...
  NILOGANFramesContext *ni_ctx = ctx->internal->priv;
  ...
  frame->buf[0] = av_buffer_create(buf, buf_size, ni_logan_frame_free, &ni_ctx->api_ctx, 0);
...
static void ni_logan_frames_uninit(AVHWFramesContext *ctx)
{
  ...
  NILOGANFramesContext *ni_ctx = ctx->internal->priv;
  AVNILOGANDeviceContext * ni_dev_ctx = ctx->device_ctx->hwctx;
  ...
  if (dev_idx != -1)
  {
    ...
    {
      ni_logan_device_session_close(&ni_ctx->api_ctx, 1, NI_LOGAN_DEVICE_TYPE_UPLOAD);
    }
  ...
  if (ni_ctx->src_session_io_data)
  {
    if (ni_ctx->src_session_io_data->data.frame.p_buffer)
    {
      ...
      ni_logan_frame_buffer_free(&ni_ctx->src_session_io_data->data.frame);
    }
  ...
  if (ni_ctx->suspended_device_handle != NI_INVALID_DEVICE_HANDLE)
  {
    ...
    ni_logan_device_close(ni_ctx->suspended_device_handle);
  }
...
static int ni_logan_frames_init(AVHWFramesContext *ctx)
{
  ...
  switch (ctx->sw_format)
  {
    case AV_PIX_FMT_YUV420P:
      ...
      ni_ctx->api_ctx.pixel_format = NI_LOGAN_PIX_FMT_YUV420P;
      break;
    case AV_PIX_FMT_YUV420P10LE:
      ni_ctx->api_ctx.bit_depth_factor = 2;
      ni_ctx->api_ctx.src_bit_depth = 10;
      ni_ctx->api_ctx.src_endian = NI_LOGAN_FRAME_LITTLE_ENDIAN;
      ni_ctx->api_ctx.pixel_format = NI_LOGAN_PIX_FMT_YUV420P10LE;
      break;
    default:
      return AVERROR(EINVAL);
  }
  ...
  else if (dev_idx >= 0)
  {
    ...
    if ((ni_ctx->rsrc_ctx = ni_logan_rsrc_allocate_simple_direct(NI_LOGAN_DEVICE_TYPE_DECODER, dev_idx)) == NULL)
    {
  ...
  ret = ni_logan_device_session_open(&ni_ctx->api_ctx, NI_LOGAN_DEVICE_TYPE_UPLOAD);
  if (ret < 0)
  {
  ...
  ret = ni_logan_device_session_init_framepool(&ni_ctx->api_ctx, pool_size);
  if (ret < 0)
  {
    return ret;
  }
  ...
  ni_ctx->src_session_io_data =  malloc(sizeof(ni_logan_session_data_io_t));
  if(!ni_ctx->src_session_io_data)
  {
    av_log(ctx, AV_LOG_ERROR, "ni_logan_frames_init(): Error alloc src_session_io_data memory\n");
    return AVERROR(ENOMEM);
  }
  memset(ni_ctx->src_session_io_data, 0, sizeof(ni_logan_session_data_io_t));
  ni_ctx->src_session_io_data->data.frame.extra_data_len = NI_LOGAN_APP_ENC_FRAME_META_DATA_SIZE;
...
static int ni_logan_hwdl_frame(AVHWFramesContext *ctx, AVFrame *dst, const AVFrame *src)
{
  ...
  ret = ni_logan_device_session_hwdl(&ni_ctx->api_ctx, p_session_data, src_surf);
  if (ret <= 0)
  {
...
static int ni_logan_hwup_frame(AVHWFramesContext *ctx, AVFrame *dst, const AVFrame *src)
{
  ...
  ni_logan_session_data_io_t * p_src_session_data = ni_ctx->src_session_io_data;
  ...
  switch (src->format)
  {
    ...
    case AV_PIX_FMT_YUV420P10LE:
      ...
      {
        dst_stride[0] = FFALIGN(NI_LOGAN_MIN_WIDTH, 32) * ni_ctx->api_ctx.bit_depth_factor;
      }
      else
      {
  ...
  ret = ni_logan_device_session_hwup(&ni_ctx->api_ctx, p_src_session_data, dst_surf);
  if (ret < 0)
  {
