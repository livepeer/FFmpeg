...
static int h264_extradata_to_annexb(AVBSFContext *ctx, const int padding, uint8_t *side_data, size_t side_size)	
{
    ...
    const uint8_t *start_addr = ctx->par_in->extradata;
    int extra_size = ctx->par_in->extradata_size;

    if (side_data) {
        extradata = side_data + 4;
        length_size = (*extradata++ & 0x3) + 1;	
        start_addr = side_data;
        extra_size = side_size;
    }
    // End of NETINT
    ...
    while (unit_nb--) {
		...
		// NETINT: change 'ctx->par_in->extradata + ctx->par_in->extradata_size' to 
		// 'start_addr + extra_size', then use current param when sequence changing
        if (extradata + 2 + unit_size > start_addr + extra_size) {
...
static int h264_mp4toannexb_init(AVBSFContext *ctx)
{
    ...
    if (!extra_size                                               ||
        ...
        ret = h264_extradata_to_annexb(ctx, AV_INPUT_BUFFER_PADDING_SIZE, NULL, 0); // NETINT: add 2 args
        if (ret < 0)
            return ret;
