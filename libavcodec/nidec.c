...
#if 0
      av_log(avctx, AV_LOG_DEBUG, "ff_decode_get_packet 1 rc: Success  sentinel: %s, s->api_ctx.internal_flush=%d\n", -1 == s->buffered_pkt.pts ? "Yes" : "No", s->api_ctx.internal_flush);
      if (-1 == s->buffered_pkt.pts)
      {
		if(s->api_ctx.internal_flush == 0){
			s->api_ctx.internal_flush = 1;
			ni_device_session_flush(&(s->api_ctx), NI_DEVICE_TYPE_DECODER);
			av_log(avctx, AV_LOG_ERROR, "ff_decode_get_packet NO need to send sentinel for now !, s->api_ctx.internal_flush=%d\n", s->api_ctx.internal_flush);
