                                  ...
                                  ni_logan_encoder_params_t *p_param)
{
  ...
  if (ret != 0)
  {
    ...
    // ff_xcoder_encode_close(avctx); will be called at codec close
    return ret;
  }
  else
  {
...
static int xcoder_setup_encoder(AVCodecContext *avctx)
{
  ...
  if (p_param->enable_vfr)
  {
    //in the vfr mode, time base can be reset to a very large value, such as 1000.
...
static int enqueue_frame(AVCodecContext *avctx, const AVFrame *inframe)
{
  ...
  if (inframe == &ctx->buffered_fme)
  {
    ...
    av_fifo_generic_write(ctx->fme_fifo, inframe, sizeof(*inframe), NULL);
  }
  else
  {
    ...
    if (ret < 0)
    {
      av_log(avctx, AV_LOG_ERROR, "Enc av_frame_ref temp_frame ERROR !!!\n");
      return ret;
    }
    // In case of double free.
    av_frame_ref(temp_frame, inframe);
    av_fifo_generic_write(ctx->fme_fifo, temp_frame, sizeof(*temp_frame), NULL);
  }
