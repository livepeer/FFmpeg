...
static int xcoder_packet_parse(AVCodecContext *avctx, XCoderH264DecContext *s, AVPacket *pkt, ni_packet_t *p_packet)
{
  ...
  while ((pkt_sei_alone || low_delay || chk_pkt) && (ptr < end))
  {
    ...
    if (AV_CODEC_ID_H264 == avctx->codec_id)
    {
      ...
      {
        got_slice = 1;
        s->vpu_reset = 0; // clear the indicator when new IDR got.
      }
      ...
      if (pkt_nal_bitmap & (NI_NAL_SPS_BIT | NI_NAL_PPS_BIT))
      {
        ...
        s->vpu_reset = 0; // clear the indicator when new sequence headers got.
        if (low_delay)
        {
...
#if 0
  // The vpu_reset indicator won't be cleared until the next sequence headers
  // come. All the AVPackets before that will be dropped except for the flush
  // operation. And it will return 0 to the caller.
  if (s->vpu_reset && !need_draining)
  {
                   ...
                   ni_frame_t *xfme)
{
  ...
  if (is_hw)
  {
    ...
    av_log(avctx, AV_LOG_DEBUG, "retrieve_frame: OUT0 data[3] i8FrameIdx=%d, device_handle=%ld bitdep=%d, WxH %d x %d\n",
           p_data3->i8FrameIdx,
  ...
  av_log(avctx, AV_LOG_DEBUG, "retrieve_frame: frame->buf[0]=%p, "
         ...
         frame->buf[0], frame->data, frame->pts, buf_size,
